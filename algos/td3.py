...
class TD3:
    ...
    def __init__(self, state_shape, action_shape, device, seed, batch_size=256, policy_noise=0.2,
        ...
        print("Action Shape in init: ", action_shape[0])
        self.ou_noise = OrnsteinUhlenbeckActionNoise(np.zeros(action_shape[0]), 0.1 * np.ones(action_shape[0]))
    ...
    def explore(self, state):
        ...
        with torch.no_grad():
            ...
            noise = torch.tensor(self.ou_noise()).unsqueeze(0).float().to(self.device)
            # print("noise shape in call: ", noise.shape)
            # print("action shape in call: ", self.actor(state).shape)
