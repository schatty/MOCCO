...
class SAC:
    ...
    def update_actor(self, states):
        ...
        if self.update_step % self.log_every == 0:
            if self.tune_alpha:
                self.wandb.log({"algo/loss_alpha": loss_alpha.item(), "update_step": self.update_step})
            self.wandb.log({"algo/loss_actor": loss_actor.item(), "update_step": self.update_step})
            self.wandb.log({"algo/alpha": self.alpha, "update_step": self.update_step})
            self.wandb.log({"algo/log_pi": log_pi.cpu().mean(), "update_step": self.update_step})
